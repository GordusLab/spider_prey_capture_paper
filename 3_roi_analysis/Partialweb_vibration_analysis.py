...
# fname = 'Z:\HsinYi\web_vibration/021424/021424_spider_prey_C001H001S0001/021424_spider_prey_C001H001S0001_nohand.avi'
# video_path = 'Z:\HsinYi\web_vibration/021424/021424_spider_prey_C001H001S0001/021424 Spider Prey C001h001s0001_nohand.mp4'
...
def roi_frame(video_path):
    ...
    np.savez(video_path.replace('.mp4',"_roi_coordinates.npz"), roi_spider_list = roi1_list, roi_fly_list = roi2_list)


    return roi1_list, roi2_list
...
def FFT_partial_web(res_fly, res_spider, res_fly_peripheral, res_spider_peripheral, data):
    ...
    ### Save the results
    ff = np.fft.fftfreq(dataFFT_fly.shape[0], 0.001)
    np.savez(fname.replace('.avi', '_roi_fft'), ff=ff, dataFFT_fly=dataFFT_fly, dataFFT_fly_p =dataFFT_fly_p , dataFFT_spider=dataFFT_spider, dataFFT_spider_p=dataFFT_spider_p)
    ...
    plt.savefig(fname.replace('.avi', '_roi_fly_fft.png'), dpi=300)
    plt.figure()
    plt.plot(ff[ff > 0], dataFFT_fly_p[ff > 0])
    plt.savefig(fname.replace('.avi', '_roi_fly_p_fft.png'), dpi=300)
    plt.figure()
    plt.plot(ff[ff > 0], dataFFT_spider[ff > 0])
    plt.savefig(fname.replace('.avi', '_roi_spider_fft.png'), dpi=300)
    plt.figure()
    plt.plot(ff[ff > 0], dataFFT_spider_p[ff > 0])
    plt.savefig(fname.replace('.avi', '_roi_spider_p_fft.png'), dpi=300)
def STFT_partial_web(res_fly, res_spider, res_fly_peripheral, res_spider_peripheral, data, window, step):
    ...
    ### Save the results
    np.savez(fname.replace('.avi', '_roi_stft'), ff=ff, f_spec_fly=f_spec_fly, f_spec_spider=f_spec_spider, f_spec_fly_p=f_spec_fly_p, f_spec_spider_p=f_spec_spider_p)
    ...
    fig = plt.figure()
    ax2 = plt.subplot()
    ax2.pcolormesh(t, ff[f_idx], f_spec_fly[f_idx], vmin=0, vmax=3000)
    plt.savefig(fname.replace('avi', '_roi_fly_stft.png'), dpi =300)
    fig = plt.figure()
    ax2 = plt.subplot()
    ax2.pcolormesh(t, ff[f_idx], f_spec_fly_p[f_idx], vmin=0, vmax=3000)
    plt.savefig(fname.replace('avi', '_roi_fly_p_stft.png'), dpi=300)
    fig = plt.figure()
    ax2 = plt.subplot()
    ax2.pcolormesh(t, ff[f_idx], f_spec_spider[f_idx], vmin=0, vmax=3000)
    plt.savefig(fname.replace('avi', '_roi_spider_stft.png'), dpi=300)
    fig = plt.figure()
    ax2 = plt.subplot()
    ax2.pcolormesh(t, ff[f_idx], f_spec_spider_p[f_idx], vmin=0, vmax=3000)
    plt.savefig(fname.replace('avi', '_roi_spider_p_stft.png'), dpi=300)


def Partialweb_vibration_analysis(fname, npydata=None):
    ...
    if os.path.exists(fname.replace('.avi', '_roi_stft.npz')):
        spec = np.load(fname.replace('.avi', '_roi_stft.npz'))
    ...
    if os.path.exists(filename):
        ...
        print(fname.replace(".avi", "_get_unet_acc_n300_modified.npy"))
    ...
    if os.path.exists(video_path.replace('.mp4',"_roi_coordinates.npz")):
        roi = np.load(video_path.replace('.mp4',"_roi_coordinates.npz"))
        roi_spider_list = roi['roi_spider_list']
        roi_fly_list = roi['roi_fly_list']
    else:
        roi_spider_list, roi_fly_list = roi_frame(video_path)
    ...
    STFT_partial_web(res_fly, res_spider, res_fly_peripheral, res_spider_peripheral, data, window=400, step=20)
...
Partialweb_vibration_analysis(fname)