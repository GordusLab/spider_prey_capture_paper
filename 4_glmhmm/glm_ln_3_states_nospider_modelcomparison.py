...
for shuffle in range(250):
    ...
    # shuffled_indicies = np.arange(42991)
    ...
    for stimulus_window in range(10,11,2):
        ...
        for states in states_label:
            ...
            train_x = np.array(linear_output_extend)[shuffled_indicies[:train_data_size]]
            train_x_fly = np.array(linear_output_vy_extend)[shuffled_indicies[:train_data_size]]
            train_y = np.array(labeled_beh_extend)[shuffled_indicies[:train_data_size]]
            test_x = np.array(linear_output_extend)[shuffled_indicies[train_data_size:]]
            test_x_fly = np.array(linear_output_vy_extend)[shuffled_indicies[train_data_size:]]
            # test_x = np.array(observation_extend)[shuffled_indicies[train_data_size:]]
            test_y = np.array(labeled_beh_extend)[shuffled_indicies[train_data_size:]]
            lr = linear_model.LogisticRegression()
            lr.fit(np.append(train_x, train_x_fly, axis=0).reshape(2, len(train_x)).transpose(), train_y)
            ...
            test_x_prob_lr = lr.predict_proba(np.append(test_x, test_x_fly, axis=0).reshape(2, len(test_x)).transpose())
            prob_states_lr_22.append(test_x_prob_lr)
            states_linear_output22.append(np.array(np.append(test_x, test_x_fly, axis=0).reshape(2, len(test_x)).transpose()))
            ...
            train_y = np.array(labeled_beh_extend)[shuffled_indicies[:train_data_size]]
            test_x = np.array(linear_output_extend)[shuffled_indicies[train_data_size:]]
            test_x_fly = np.array(linear_output_fly_extend)[shuffled_indicies[train_data_size:]]
            test_x_vx = np.array(linear_output_vx_extend)[shuffled_indicies[train_data_size:]]
            test_x_vy = np.array(linear_output_vy_extend)[shuffled_indicies[train_data_size:]]
            # test_x = np.array(observation_extend)[shuffled_indicies[train_data_size:]]
            test_y = np.array(labeled_beh_extend)[shuffled_indicies[train_data_size:]]
            lr = linear_model.LogisticRegression()
        ...
        prob_temp_22 = np.array(prob_states_lr_22)
        prob_temp_22 = np.swapaxes(prob_temp_22, 0, 1)

        idx_list_22 = []
        for i in range(len(prob_temp_22)):
            idx_list_22.append(np.where(prob_temp_22[i, :] == np.max(prob_temp_22[i, :]))[1][0])
        idx_list_22 = np.array(idx_list_22)
        accuracy = sum((test_y == np.array(idx_list_22).astype(int)).astype(int)) / len(idx_list_22)
        # print("Two features accuracy-vy = ", accuracy)
        # print('Two features F score-vy = ', f1_score(test_y, np.array(idx_list_22).astype(int), average='micro'))
        accuracy_list22.append(accuracy)
    ...
    for i in range(4):
        if i==0:
            f1_1.append(f1_score(test_y[0: time_count[12]-count], np.array(idx_list[0: time_count[12]-count]).astype(int), average='micro'))
            f1_2.append(f1_score(test_y[0: time_count[12]-count],
                                 np.array(idx_list_2[0: time_count[12]-count]).astype(int), average='micro'))
            f1_22.append(f1_score(test_y[0: time_count[12]-count],
                                 np.array(idx_list_22[0: time_count[12]-count]).astype(int), average='micro'))

            f1_4.append(f1_score(test_y[0: time_count[12]-count],
                                 np.array(idx_list_4[0: time_count[12]-count]).astype(int), average='micro'))

        else:
            f1_1.append(f1_score(test_y[time_count[i+11]-count: time_count[i+12]-count], np.array(idx_list[time_count[i+11]-count: time_count[i+12]-count]).astype(int),
                                 average='micro'))
            f1_2.append(f1_score(test_y[time_count[i+11]-count: time_count[i+12]-count],
                                 np.array(idx_list_2[time_count[i+11]-count: time_count[i+12]-count]).astype(int), average='micro'))
            f1_22.append(f1_score(test_y[time_count[i+11]-count: time_count[i+12]-count],
                                  np.array(idx_list_22[time_count[i+11]-count: time_count[i+12]-count]).astype(int), average='micro'))

            f1_4.append(f1_score(test_y[time_count[i+11]-count: time_count[i+12]-count],
                                 np.array(idx_list_4[time_count[i+11]-count: time_count[i+12]-count]).astype(int), average='micro'))



np.save('F1s_onefeature.npy', np.array(f1_1))
np.save('F1s_twofeature_vx.npy', np.array(f1_2))
np.save('F1s_twofeature_vy.npy', np.array(f1_22))
np.save('F1s_threefeature.npy', np.array(f1_4))
...
stat, p = kruskal(f1_1, f1_2, f1_22, f1_4
                  )
