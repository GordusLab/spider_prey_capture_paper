...
def Wavelet_transform(fname, filename, dirOut, joints):
    ...
    fnameOut = dirOut + '/wavelet/' + fname + s + '_wavelet.mp4'
    ...
    for i in range(len(joints)):
        dataLeg_x[:, i] = datarelRel[:, joints[i], 0]
        dataLeg_y[:, i] = datarelRel[:, joints[i], 1]
    ...
    amplitudes_x = np.flipud(amplitudes_x)
    amplitudes_y = np.flipud(amplitudes_y)

    np.savez(dirOut + '/wavelet/' + fname + s + '_wavelet.npz', amplitudes_x, amplitudes_y, f)
    ...
    ax2 = plt.subplot(221)
    lm2 = ax2.pcolormesh(np.array(list(np.arange(0, amplitudes_x.shape[0]))),
                         np.array(list(np.arange(0, 50 * len(joints)))), amplitudes_x.T, vmax=0.5)
    # ax2.xaxis.set_ticks([])
    ...
    ax3 = plt.subplot(222)
    lm3 = ax3.pcolormesh(np.array(list(np.arange(0, amplitudes_y.shape[0]))),
                         np.array(list(np.arange(0, 50 * len(joints)))), amplitudes_y.T, vmax=0.5)
    ...
    fig.tight_layout()
    plt.savefig(dirOut + '/wavelet/' + fname + s + '_wavelet.png')
