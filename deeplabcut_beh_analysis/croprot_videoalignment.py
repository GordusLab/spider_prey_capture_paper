...
#joints= np.load(filename.replace(".h5", "_interpolation.npy"))
    ...
    out = cv2.VideoWriter(
        vid_name.split('/videos')[0] + '/videos/aligned' + vid_name.split('/videos')[1].replace(".mp4",
                                                                                                "_croprotaligned.mp4"), fourcc, 100, (n_w, n_h))
    ...
    np.save(vid_name.split('/videos')[0] + '/videos/new_prey/aligned' + vid_name.split('/new_prey')[1].replace(".mp4", "_croprotaligned.npy"),joints_new)
